...
const transcriptionPlaceholder = document.querySelector("#transcriptionPlaceholder");
...
const setupAudioRecording = async () => {
  try {
    console.log("[AUDIO] setupAudioRecording called");
    ...
    console.log("[AUDIO] Got mediaStream with", mediaStream.getTracks().length, "tracks");
    ...
    console.log("[AUDIO] audioContext created:", audioContext);
    console.log("[AUDIO] audioContext type:", audioContext.constructor.name);
    console.log("[AUDIO] audioContext methods:", Object.getOwnPropertyNames(Object.getPrototypeOf(audioContext)).filter(m => typeof audioContext[m] === 'function'));
    
    if (!audioContext || typeof audioContext.createAnalyser !== 'function') {
      throw new Error("AudioContext is invalid or missing methods");
    }
    
    analyser = audioContext.createAnalyser();
    analyser.fftSize = 2048;
    console.log("[AUDIO] Analyser created, checking for createMediaStreamSource...");
    console.log("[AUDIO] typeof createMediaStreamSource:", typeof audioContext.createMediaStreamSource);
    
    if (typeof audioContext.createMediaStreamSource !== 'function') {
      throw new Error(`createMediaStreamSource is not available. Available methods: ${Object.getOwnPropertyNames(Object.getPrototypeOf(audioContext)).join(', ')}`);
    }
    ...
    console.log(`[AUDIO] Audio setup: context=${audioContext.state}, fftSize=${analyser.fftSize}`);
    
    // Create MediaRecorder
    mediaRecorder = new MediaRecorder(mediaStream);
    recordedChunks = [];
    addLog(`✓ MediaRecorder created - mimeType: ${mediaRecorder.mimeType}`);
    console.log(`[AUDIO] MediaRecorder: state=${mediaRecorder.state}, mimeType=${mediaRecorder.mimeType}`);

    mediaRecorder.ondataavailable = (event) => {
      console.log("[AUDIO] ondataavailable called, data size:", event.data.size);
      recordedChunks.push(event.data);
    };

    mediaRecorder.onstop = async () => {
      console.log("[AUDIO] onstop called");
      ...
      if (audioBlob.size > 100) { // Only send if audio is not empty
        ...
        try {
          ...
          console.log("[AUDIO] Sending audio.recorded event with languages:", languages);
          sendEvent("audio.recorded", {
            audioBase64,
            mimeType: "audio/webm",
            expectedUserLanguage: languages.expectedUserLanguage,
            agentLanguage: languages.agentLanguage
          });
          console.log("[AUDIO] audio.recorded event sent successfully");
        } catch (error) {
          console.error("[AUDIO] Error sending audio:", error);
          addLog(`Failed to send audio: ${error instanceof Error ? error.message : "unknown error"}`);
          updateChatStatus("Failed to send audio");
        }
      } else {
        console.log("[AUDIO] Audio blob too small, not sending");
      }
    ...
    console.log("[AUDIO] setupAudioRecording completed successfully");
  } catch (error) {
    console.error("[AUDIO] Error in setupAudioRecording:", error);
    mediaRecorder = null;
...
const startVoiceListening = async () => {
  try {
    ...
    if (!mediaRecorder) {
      console.log("[VOICE] mediaRecorder not set, calling setupAudioRecording");
      addLog("Requesting microphone access...");
      await setupAudioRecording();
    }
    
    if (!mediaRecorder) {
      console.log("[VOICE] mediaRecorder still not set after setupAudioRecording");
    ...
    console.error("[VOICE] Error in startVoiceListening:", error);
    isListening = false;
  ...
  socket.addEventListener("message", async (event) => {
    ...
    switch (message.type) {
      ...
      case "transcript.final":
        ...
        // Add the user's transcribed message to the conversation
        addMessageToConversation(message.payload.text, userProfile);
...
profileSubmitButton.addEventListener("click", () => {
  ...
  // Auto-set session mode based on profile
  if (userProfile === "support") {
    sessionMode = "pool"; // Support agents wait for users
    userStatus = "free";
  } else {
    sessionMode = "create"; // End users create/join session
  }
